...
#include <fstream>
#include <string>
#include <vector> // for interpreting lines of input file
#include <sstream> // for interpreting lines of input file
...
///////////////// USER ADDITIONS
bool* decToBin(int dec); // converts decimal number to new bool*
void printRegistry(); // prints contents of Registry
void printInstructionMemory();
void printDataMemory();
void assignPC(bool* assignment); // helper function for putting value in PC
std::vector<std::string> split(std::string input); // splits input based on strings (for interpreting input file)
void copy_8bit(bool* input1, bool* input2, int offset); // bitwise copy of array with source offset (input1[i] = input2[i+offset])
bool* push_instruction(bool first[8],bool second[8],bool third[8],bool fourth[8], int index); //concatenates different parts of instruction array
bool* decToBin_16b(int dec); //decimal number to new 16b bool*
bool is_rtype(bool* name); // determine if a name corresponds to a r-type value
